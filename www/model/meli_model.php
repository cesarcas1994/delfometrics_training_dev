...
class Meli
{
      ...
      if ($permalinks_array != null) {
        ...
        */
        ...
        foreach ($sold_quantity_scraping_array as $key => $value) {
          ...
          if (($sold_quantity_scraping_array[$key] != null) || ($sold_quantity_scraping_array[$key] != "")) {          
    ...
    */      
    ...
    public function get_items_features($object_array, $features_to_obtain, $country_id = "MLM", $coin = "MXN"){
      ...
      //conversion
          ...
          //sold_quantity
            $sold_quantity_array = $this -> get_sold_quantity_array($object_array, $url_azure_cloud = "https://mlconexionmeli.azurewebsites.net/api/ScrapingHTTP2?code=xQ27rQhYFjXRhKTbhtUaJ3wXudhazCicawtobyzoTuD1a25u96HgSg=="/*"https://tablesoldquantity.azurewebsites.net/api/ScrapingHTTP?code=9TtiE9aIhq1apfSMbolmwZZgXOcyBXYFos0Zdn1zga2v/gaddrDBqw=="*/);
            var_dump('sold_quantity_array');
            var_dump($sold_quantity_array);
    ...
    public function fulfillment_calculator_item_id($item_id){
      ...
      if ($sold_quantity_array === null) {
        $sold_quantity_array = $this -> get_sold_quantity_array($item_object, $url_azure_cloud = "https://mlconexionmeli.azurewebsites.net/api/ScrapingHTTP2?code=xQ27rQhYFjXRhKTbhtUaJ3wXudhazCicawtobyzoTuD1a25u96HgSg=="/*"https://tablesoldquantity.azurewebsites.net/api/ScrapingHTTP?code=9TtiE9aIhq1apfSMbolmwZZgXOcyBXYFos0Zdn1zga2v/gaddrDBqw=="*/);
        var_dump('sold_quantity_array');
        var_dump($sold_quantity_array);
      }
    ...
    public function fulfillment_calculator_seller_id($seller_id){
        ...
        //sold_quantity for days
            ...
            if ($sold_quantity_array === null) {
              $sold_quantity_array = $this -> get_sold_quantity_array($seller_articles_total, $url_azure_cloud = "https://mlconexionmeli.azurewebsites.net/api/ScrapingHTTP2?code=xQ27rQhYFjXRhKTbhtUaJ3wXudhazCicawtobyzoTuD1a25u96HgSg=="/*"https://tablesoldquantity.azurewebsites.net/api/ScrapingHTTP?code=9TtiE9aIhq1apfSMbolmwZZgXOcyBXYFos0Zdn1zga2v/gaddrDBqw=="*/);
              var_dump('sold_quantity_array');
              var_dump($sold_quantity_array);
            }
    ...
    */ 
    ...
    public function data_extractor($params){
      ...
      set_time_limit(0); //extend the time limit to infinite.
      ...
      //encontrar "el numero" de las categorias en MLM_categorys father, (la de 11000 categorias inmediatas hijas menores)

      /*
      * default! $country_base = "MLM"
      */     
      ...
      if ($country_base = "MLM") {
        $fathers_of_children_categories = $this -> get_all_children_categories_mexico_local();
      }
      else{
        return "just $country_base for MLM mexico";
      }
      ...
      $count = 0;
      foreach ($fathers_of_children_categories as $key_1 => $value_1) {
        
        foreach ($categories_ids_array as $key_2 => $value_2) {
          
          if($fathers_of_children_categories[$key_1] == $categories_ids_array[$key_2])
          {

            $min_children_category_array[$count] = $key_1;
            $max_children_category_array[$count] = $key_1;
            $count = $count + 1; 
          }
        }
      }

      var_dump("min_children_category_array and max_children_category_array");
      var_dump($min_children_category_array);
      var_dump($max_children_category_array);
      
      */

      //despues de haber obtenido numeros ej:525,921,9295,4152. Hacer un ciclo donde se introducen como $min_children_category=525, $max_children_category=525; $min_children_category=921, $max_children_category=921
      /*
      foreach ($min_children_category_array as $key => $value) {
        
        $min_children_category = $min_children_category_array[$key];
        $max_children_category = $max_children_category_array[$key];

        $start_time = microtime(true);

        $cousin_and_total_items = $this -> comparison_model_prepare_to_getfeatures_v2($min_children_category, $max_children_category, $plus= null, $force_calculation_under_1000 = true, $country_base = "MLM");

        $this -> get_items_features_unified_v2($min_children_category, $max_children_category, $cousin_and_total_items["category_cousins"], $country_base = "MLM");
      ...
      */
      //correr todo lo demás dentro de este ciclo.

      //Pendiente nueva via, si funciona borrar todo lo demás

      //Por aca me quedé 31 de agosto 2020<
      //1- modificar en mercadolibre_comparador comparison_model_prepare_to_getfeatures_v4 y get_items_features_unified_v4 para que trabajen con data del exterior y ahorrar trabajo conectandolo con lo primero echo.
      //2- traer aca la funcion.
      ...
      $send_to_ml = $this -> get_items_features_unified_v4($cousin_and_total_items["category_cousins"], $cousin_and_total_items["category_items"], $country_base = "MLM");

      $end_time = microtime(true);

      // TODO Pendiente crear una azure function -> mandar estos datos a una tabla en azure storage registrando los datos, una segunda tabla debe ser creada que agarre los valores promedio de esta tabla(trabajo que hará otra azure function).

      echo "<br> indirect call - data extractor moder - time rounded: " . round($end_time - $start_time, 6) . " seconds";
      echo "<br>download speed : " . round($cousin_and_total_items["total_items"]/($end_time - $start_time), 4) . " art/seconds";

      $save_data["categorytraining_array"] = $category_array;
      $save_data["categorytraining_time_sec"] = round($end_time - $start_time, 6);
      $save_data["categorytraining_art_per_sec"] = round($cousin_and_total_items["total_items"]/($end_time - $start_time), 4);

      var_dump("before save_azure_table_data_extractor_performance");
      var_dump($save_data);

      /*if ($send_to_ml != "fail training") {
        $this -> save_azure_table_data_extractor_performance($save_data);
      }
      */
      ...
      // obtein items in cousins categories  
        ...
        foreach ($category_cousins as $country_base_category => $value) {
          ...
          foreach ($category_cousins[$country_base_category] as $country_id => $value) {
            ...
            $result_category_items[$country_id] = $this -> get_full_article_array($array_to_search);    
            $cousin_and_total_items["category_items"][$country_base_category][$country_id] = $result_category_items[$country_id]; 
          }
    ...
    public function get_items_features_unified_v4($category_cousins, $category_items, $country_base){
      ...
      // get_items_features
        ...
        foreach ($category_cousins as $country_base_category => $value) {
          ...
          $result_general_category_items = $category_items[$country_base_category];
          ...
          $send_to_ml = $this -> csv_to_machinelearning_v2($items_features_category_items, $country_base_category);
          unset($items_features_category_items);
        }
    ...
    public function save_azure_table_data_extractor_performance($data_performance){

      $Azure_function_save_data_extractor_performance = "https://mlconexionmeli.azurewebsites.net/api/table_data_extractor_performance?code=1z67WvHYl8vyG6VUndMrPt8LLggSA5VlPuuQzJPPn96hFbC15qsxOA==";

      $body = array(
        "categorytraining_array" => $data_performance["categorytraining_array"],
        "categorytraining_time_sec" => $data_performance["categorytraining_time_sec"],
        "categorytraining_art_per_sec" => $data_performance["categorytraining_art_per_sec"]
      );
      ...
      $uri = $Azure_function_save_data_extractor_performance;
